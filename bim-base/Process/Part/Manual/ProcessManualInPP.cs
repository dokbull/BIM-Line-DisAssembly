...
namespace bim_base
{
    public class ProcessManualInPP : CProcess
    {
        public enum TARGET
        {
            READY,
        ...
        TEACH_POS m_teachPos = TEACH_POS.PICK_PP_WAIT;

        bool m_moveAxisZ = false;

        public ProcessManualInPP(ProcessMain procMain) : base(procMain)
        {
        ...
        public override void run()
        {
            ...
            if (m_step != m_agoStep)
                Debug.debug("ProcessManualInPP::run STEP:" + m_step);
            ...
            switch (m_step)
            {
                ...
                case STEP.MOVE_Y_TARGET:
                    {
                        if (m_teachPos == TEACH_POS.PICK_PP_PLACE)
                        {
