...
public class TPSerial
{
    ...
    public TPSerial(SerialPort serialPort)
    {
    ...
    private void run()
    {
        Debug.debug("Q6TPSerial::run START COM:" + m_serialPort.PortName);

        while (true)
        {
            if (m_stop)
            {
                Debug.debug("Q6TPSerial::run STOP");
                break;
            }
            ...
            {
                Debug.critical(ex,
                    "Q6TPSerial::run port:" + m_serialPort.PortName);
            }
        ...
        Debug.debug("Q6TPSerial::run END");
    }
            ...
            for (int i = 0; i < buff.Length; i++)
            {
                ...
                if (buff[i] == ETX)
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        Util.memcpy(ref data, 0, readData, 0, dataLength);
    ...
    private void processIncmonig(byte[] data)
    {
        ...
        Debug.debug("Q6TPSerial::processIncoming data:" + text);
