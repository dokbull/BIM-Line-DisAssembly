...
public class CSerialBCR_Zebra
{
    ...
    public CSerialBCR_Zebra(SerialPort serialPort)
    {
    ...
    private void run()
    {
        ...
        while (true)
        {
            ...
            {
                Debug.critical(ex,
                    "CSerialBCR::run port:" + m_serialPort.PortName);
            }
    ...
    private bool recvProcess()
    {
        lock (m_recvBuffer)
        {
            byte[] readData = new byte[m_recvBuffer.size()];

            if (readData.Length > 0)
            {
                m_recvBuffer.read(ref readData, m_recvBuffer.size());

                string text = ASCIIEncoding.ASCII.GetString(readData);

                processIncmonig(readData);
            }
        }
