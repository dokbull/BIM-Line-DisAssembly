...
public class CSerialKeyence_DLRS1A
{
    static byte CR = 0x0D;
    static byte LF = 0x0A;
    ...
    public CSerialKeyence_DLRS1A(SerialPort serialPort)
    {
    ...
    private void run()
    {
        Debug.debug("CSerialKeyence_DLRS1A::run START COM:" + m_serialPort.PortName);

        while (true)
        {
            if (m_stop)
            {
                Debug.debug("CSerialKeyence_DLRS1A::run STOP");
                break;
            }
            ...
            {
                Debug.critical(ex,
                    "CSerialKeyence_DLRS1A::run port:" + m_serialPort.PortName);
            }
        ...
        Debug.debug("CSerialKeyence_DLRS1A::run END");
    }
    ...
    private void processIncmonig(byte[] data)
    {
        ...
        Debug.debug("CSerialKeyence_DLRS1A::processIncoming port:" + m_serialPort.PortName + " data:" + text);
