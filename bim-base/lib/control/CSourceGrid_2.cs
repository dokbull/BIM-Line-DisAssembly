...
{
    public int headHeight = 36;
    public int headWith = 50;
        ...
        bool rowScrollBar = false)
    {
        ...
        if (rowAutoSize == true)
        {
            ...
            for (int r = 0; r < row; r++)
            {
                if (r == 0)
                {
                    this.Rows[r].Height = headHeight;
                }
                else
                {
                    this.Rows[r].Height = (height - headHeight) / (row-1);
                }
            }
        ...
        if (colAutoSize == true)
        {
            for (int c = 0; c < col; c++)
            {
                if (c == 0)
                {
                    this.Columns[c].Width = headWith;
                }
                else
                { 
                    this.Columns[c].Width = (width - headWith)/ (col - 1);
                }
            }
               
        }
    }
    public string getvaluestr(int _row,int _col) 
    {
        return this.GetCell(_row, _col).ToString();
    }
    public Color GetBackColor(int r, int c)
    {
        return viewCell(r, c).BackColor;
    }
    ...
    {
        this.Selection.FocusRow(_index);
    }
